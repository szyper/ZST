...
namespace _9_pliki
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var library = new LibraryManager("books.csv", logger);

            logger.Log("=== Uruchomiono program biblioteki ===", Logger.LogType.Info);
        ...
        private static void RemoveBook()
        {
            ...
            Console.Write($"\nCzy na pewno chcesz usunąć książkę:\n{FormatBook(bookToRemove)}\n(t = tak / inny klawisz = anuluj)? ");

            string confirm = Console.ReadLine()?.Trim().ToLower();

            if (confirm != "t")
            {
                Log("Anulowano usuwanie książki", LogType.Info);
                return;
            }
            ...
            Log($"Usunięto książkę: {title} - {author}", LogType.Success);
        }
