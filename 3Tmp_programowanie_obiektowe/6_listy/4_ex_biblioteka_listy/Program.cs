...
namespace _6_biblioteka_listy
{
    internal class Program
    {
        ...
        {
            Console.OutputEncoding = Encoding.UTF8; // obsługa polskich znaków w konsoli

            bool running = true; // flaga kontrolująca główną pętlę
        ...
        private static void SaveToFile()
        {
            ...
            {
                Log($"Błąd zapisu do pliku: {ex.Message}", LogType.Error);
            }
        }
        ...
        private static void DisplayBooks(List<string> books, string title)
        {
            ...
            if (!books.Any())
            {
                Log("Brak książek", LogType.Info, false);
                return;
            }
        ...
        private static void AddBook()
        {
            ...
            {
                Log($"Nieprawidłowy rok: '{yearInput}' (nie jest liczbą)", LogType.Error);
                return;
            }
            int currentYear = DateTime.Now.Year;
            int maxYear = currentYear + 1;
            if (year < 1440 || year > maxYear)
            {
                Log($"Nieprawidłowy rok: {year} (dopuszczalny zakres: {1440}-{maxYear})");
                return;
            }
            ...
            // unikalność (dokończyć)
            string key = $"{title.ToLowerInvariant()};{author.ToLowerInvariant()}";
            bool alreadyExists = library.Any(line => line.ToLowerInvariant().StartsWith(key));

            if (alreadyExists)
            {
                Log($"Książka '{title}' autorstwa {author} już istnieje na liście!", LogType.Error);
                return;
            }
            
            string csvLine = $"{Escape(title)};{Escape(author)};{year};{Escape(genre)}";
        ...
        // uproszczona wersja bez escapowania, bez Trim()
        ...
        {
            Console.WriteLine("\n" + new string('=', 50));
            Console.WriteLine("    SYSTEM ZARZĄDZANIA BIBLIOTEKĄ KSIĄŻEK");
            Console.WriteLine("\n" + new string('=', 50));
            ...
            Console.WriteLine("8. Wyświetl wszystkie ksiązki");
