...
namespace _7_słowniki
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // wczytywanie słownika z pliku slownik.txt
            LoadDictionaryFromFile(translations, "slownik.txt");
            ...
            while (true)
            {
                ...
                switch (choice)
                {
                    ...
                    case "8":
                        SaveDictionaryToFile(translations, "slownik.txt");
                        SavedHistoryToFile(history, "history.txt");
                        break;

                    case "9":
                        Console.Write("Czy na pewno zapisać słownik i historię? (t / n): ");
                        ...
                        {
                            Console.WriteLine("Zapis anulowany");
                        }
        ...
        private static void LoadDictionaryFromFile(Dictionary<string, List<string>> translations, string fileName)
        {
            if (!File.Exists(fileName))
            {
                Console.WriteLine($"Plik {fileName} nie istnieje - rozpoczynamy z pustym słownikiem");
                return;
            }
            ...
            try
            {
                ...
                { 
                    ...
                    string valuesPart = trimmedLine.Substring(separatorIndex + 1).Trim();
                    ...
                    {
                        string[] rawParts = valuesPart.Split(',');

                        List<string> cleanTranslations = new List<string>();
                        ...
                        foreach (var part in rawParts)
                        {
                            ...
                            {
                                cleanTranslations.Add(t);
                            }
                        }

                        foreach (string translation in cleanTranslations)
                        {
                            bool alreadyExists = false;

                            foreach (string existing in translations[key])
                            {
                                if (string.Equals(existing, translation, StringComparison.OrdinalIgnoreCase))
                                {
                                    alreadyExists = true; 
                                    break;
                                }
                            ...
                            if (!alreadyExists) 
                            {
                                translations[key].Add(translation);
                                loadedCount++;
                            }
                        }
                ...
                Console.WriteLine($"Wczytano słownik z pliku: {fileName} ({translations.Count} słów, {loadedCount} tłumaczeń)");

            }
            catch (Exception ex)
            {
                Console.WriteLine($"Błąd podczas wczytywania pliku {fileName} {translations.Count} słów");
                Console.WriteLine("Rozpoczynamy z pustym słownikiem");
            }
        }

        private static void SavedHistoryToFile(List<string> history, string fileName)
        {
            ...
            Console.WriteLine($"\nHistoria dopisana do pliku {fileName}\n");
        }

        private static void SaveDictionaryToFile(Dictionary<string, List<string>> translations, string fileName)
        {
            using (StreamWriter writer = new StreamWriter(fileName, true, Encoding.UTF8))
            {
                foreach (var pair in translations.OrderBy(p => p.Key, StringComparer.OrdinalIgnoreCase))
                {
                    var sorted = pair.Value.OrderBy(t => t, StringComparer.OrdinalIgnoreCase);
                    writer.WriteLine($"{pair.Key}: {string.Join(", ", sorted)}");
                }
            }
            Console.WriteLine($"\nSłownik dopisany do pliku {fileName}\n");
        }
