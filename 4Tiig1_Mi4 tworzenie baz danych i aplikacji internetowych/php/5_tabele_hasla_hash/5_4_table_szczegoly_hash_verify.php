...
<body>
    <?php
        ...
        if (isset($_POST["id"])) {
            ...
            // formularz zgadywania hasła
            echo <<< FORM
                <hr>
                <form method='post'>
                    <input type='hidden' name='check_id' value='$id'>
                    <label>Spróbuj zgadnąć hasło: </label>
                    <input type='password' name='guess_password'>
...
FORM;
        ...
        // jeśli wysłano formularz zgadywania hasła
        if (isset($_POST['check_id']) && isset($_POST['guess_password'])) {
            $id = $_POST['check_id'];
            $guess = $_POST['guess_password'];
            $user = $users[$id];
            $hash = $user['password'];
            ...
            if (password_verify($guess, $hash)) {
                echo "<p style='color:green;'>Hasło poprawne</p>";
            } else{
